...
exports.getPopularCategories = exports.getBooksByCategory = exports.deleteCategory = exports.updateCategory = exports.getCategoryBySlug = exports.getCategoryById = exports.getSubCategories = exports.getMainCategories = exports.getAllCategories = exports.createCategory = void 0;
...
const createCategory = async (req, res) => {
    try {
        ...
        let newCategory = new category_model_1.Category({
            name,
            description,
            slug,
            image,
        ...
        if (parentCategory && mongoose_1.default.Types.ObjectId.isValid(parentCategory)) {
            const parent = await category_model_1.Category.findById(parentCategory);
            if (!parent) {
...
const getCategoryById = async (req, res) => {
    try {
        ...
        const category = await category_model_1.Category.findById(id).populate('parentCategory', 'name slug');
        if (!category) {
...
const getCategoryBySlug = async (req, res) => {
    try {
        ...
        const category = await category_model_1.Category.findOne({ slug }).populate('parentCategory', 'name slug');
        if (!category) {
            return res.status(404).json({ message: 'Không tìm thấy danh mục' });
        }
        res.json(category);
...
const updateCategory = async (req, res) => {
    try {
        ...
        if (parentCategory) {
            if (mongoose_1.default.Types.ObjectId.isValid(parentCategory)) {
                const parent = await category_model_1.Category.findById(parentCategory);
                if (!parent) {
                    return res.status(404).json({ message: 'Không tìm thấy danh mục cha' });
                }
...
const deleteCategory = async (req, res) => {
    try {
        ...
        const hasSubcategories = await category_model_1.Category.exists({ parentCategory: id });
        if (hasSubcategories) {
...
const getBooksByCategory = async (req, res) => {
    try {
        ...
        if (!category.isSubCategory) {
            const subCategories = await category_model_1.Category.find({ parentCategory: category._id });
            categoryIds = [...categoryIds, ...subCategories.map(cat => cat._id)];
        }
        // Tìm sách thuộc danh mục và danh mục con
        const books = await book_model_1.Book.find({ category: category.name })
            .skip(skip)
            .limit(limit)
            .sort({ createdAt: -1 });
        const totalBooks = await book_model_1.Book.countDocuments({ category: category.name });
        res.json({
            category,
            books,
            currentPage: page,
...
const getPopularCategories = async (req, res) => {
    try {
        ...
        const categoryCounts = await book_model_1.Book.aggregate([
        ...
        const categories = await category_model_1.Category.find({ name: { $in: categoryNames } });
